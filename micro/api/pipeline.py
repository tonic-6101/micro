...
def get_pipeline(show_closed: bool = False) -> dict:
	"""Get pipeline stages with grouped customers for Kanban view."""
	...
	for stage in stages:
		customers = frappe.get_list(
			"Micro Customer",
			filters={"pipeline_stage": stage.name},
			fields=customer_fields,
			order_by="modified desc",
		)
		result.append({"stage": stage, "customers": customers})

	# Customers without a pipeline stage
	unassigned = frappe.get_list(
		"Micro Customer",
		filters={"pipeline_stage": ["in", ["", None]]},
		fields=customer_fields,
		order_by="modified desc",
	)
...
def move_customer(customer_id: str, stage_id: str) -> dict:
	...
	frappe.has_permission("Micro Customer", "write", throw=True)

	if not frappe.db.exists("Micro Customer", customer_id):
		frappe.throw(_("Customer not found"))

	if not frappe.db.exists("Micro Pipeline Stage", stage_id):
		frappe.throw(_("Pipeline stage not found"))

	frappe.db.set_value("Micro Customer", customer_id, "pipeline_stage", stage_id)
